...
$campo_pesquisa = $_GET['campo_pesquisa'] ?? 'processo_codigo';
...
try {
    ...
    try {
        ...
        $situacoes_processo = $stmt_sit->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        $message = "Erro ao carregar filtros de situação de processo.";
        $message_type = "error";
    }

    $params = [];
    $where_clauses = [];
    $where_clauses[] = "DATE(p.PRS_DATA_EMISSAO) BETWEEN ? AND ?";
    $params[] = $data_inicio;
    $params[] = $data_fim;

    if (!empty($filtro_status)) {
        $where_clauses[] = "p.PRS_PSI_CODIGO = ?"; 
        $params[] = $filtro_status;
    }

    if (!empty($valor_pesquisa)) {
        switch ($campo_pesquisa) {
            case 'processo_codigo':
                $where_clauses[] = "p.PRS_CODIGO = ?";
                $params[] = $valor_pesquisa;
                break;
            case 'os_codigo':
                $where_clauses[] = "p.PRS_ORS_CODIGO = ?";
                $params[] = $valor_pesquisa;
                break;
            case 'placa':
    ...
    $sql_count = "
        ...
        LEFT JOIN PROCESSO_SITUACAO ps ON p.PRS_PSI_CODIGO = ps.PSI_CODIGO
        $where_sql
    ";
    ...
    $sql_lista = "
        SELECT 
            ...
            v.PLACA_UF,
            ps.PSI_DESCRICAO AS SITUACAO_NOME -- A 'palavra'
        FROM PROCESSO p
        LEFT JOIN CLIENTE c ON p.PRS_CPF_CNPJ = c.CPF_CNPJ
        LEFT JOIN VEICULO v ON p.PRS_VEI_CODIGO = v.CODIGO
        LEFT JOIN PROCESSO_SITUACAO ps ON p.PRS_PSI_CODIGO = ps.PSI_CODIGO
    ...
    $message = "Erro ao consultar Processos: " . $e->getMessage();
    $message_type = "error";
}
...
<head>
    <meta charset="UTF-8">
    <title>Relatório de Processos - Lindomar Despachante</title>
...
<body>
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <form method="GET" action="relatorio_processos.php">
                <fieldset class="search-box">
                    <legend>Filtrar Processos (por Data de Emissão)</legend>
                    ...
                    <div class="form-group">
                        ...
                        <select name="campo_pesquisa">
                            <option value="processo_codigo" <?= $campo_pesquisa == 'processo_codigo' ? 'selected' : '' ?>>Nº Processo</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status">
                            ...
                            <?php foreach ($situacoes_processo as $sit): ?>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php if(empty($processos)): ?>
                            <tr>
                                <td colspan="7" style="text-align: center;">Nenhum Processo encontrado para estes filtros.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach($processos as $proc): ?>
                                <tr>
                                    <td><?= htmlspecialchars($proc['PRS_CODIGO']) ?></td>
                                    <td><?= htmlspecialchars($proc['PRS_ORS_CODIGO']) ?></td>
                                    <td class="col-data"><?= $proc['PRS_DATA_EMISSAO'] ? date('d/m/Y', strtotime($proc['PRS_DATA_EMISSAO'])) : 'N/A' ?></td>
                                    <td class="col-cliente"><?= htmlspecialchars($proc['CLIENTE_NOME'] ?? 'N/A') ?></td>
                                    <td><?= htmlspecialchars($proc['PLACA_UF'] ?? 'N/A') ?></td>
                                    <td><?= htmlspecialchars($proc['SITUACAO_NOME'] ?? 'N/A') ?></td>
                                    <td class="col-valor"><?= number_format($proc['PRS_VALOR_TOTAL'], 2, ',', '.') ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
            ...
            <div class="pagination">
                <span class="paginacao">Página <?= $page ?> de <?= $total_pages ?> (Total de <?= $total_records ?> Processos)</span>
            </div>
