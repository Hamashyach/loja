...
try {
    $params = [];
    $where_clauses = [];
    $where_clauses[] = "DATE(v.VEI_DT_CADASTRO) BETWEEN ? AND ?";
    $params[] = $data_inicio;
    $params[] = $data_fim;
    if (!empty($valor_pesquisa)) {
        switch ($campo_pesquisa) {
            case 'renavam':
                $where_clauses[] = "v.RENAVAM LIKE ?";
                $params[] = '%' . $valor_pesquisa . '%';
                break;
            case 'chassi':
                $where_clauses[] = "v.CHASSI LIKE ?";
                $params[] = '%' . $valor_pesquisa . '%';
                break;
            case 'proprietario':
    ...
    $sql_count = "
        SELECT COUNT(*) 
        FROM VEICULO v 
        LEFT JOIN CLIENTE c ON v.CPF_CNPJ = c.CPF_CNPJ 
        $where_sql
    ";
    ...
    $sql_lista = "
        ...
        $where_sql
        ORDER BY 
            v.VEI_DT_CADASTRO DESC -- Mais recentes primeiro
        LIMIT $limit OFFSET $offset
    ";

    $stmt_lista = $conn->prepare($sql_lista);
    $stmt_lista->execute($params);
    $veiculos = $stmt_lista->fetchAll(PDO::FETCH_ASSOC);

    // --- LÓGICA DE NAVEGAÇÃO---
    $nav_params = $_GET; unset($nav_params['page']);
    $query_string = http_build_query($nav_params);
    $link_base = "relatorio_veiculos.php?" . ($query_string ? $query_string . '&' : ''); 
    ...
    $message = "Erro ao consultar veículos: " . $e->getMessage();
    $message_type = "error";
}
...
<head>
    <meta charset="UTF-8">
    <title>Relatório de Entrada de Veículos - Lindomar Despachante</title>
    ...
    <div class="main-container">
       <aside class="sidebar">
        ...
        <main class="main-content">
            <h1>Relatório de Entrada de Veículos</h1>
            ...
            <form method="GET" action="relatorio_veiculos.php">
                <fieldset class="search-box">
                    <legend>Filtrar Veículos (por Data de Cadastro)</legend>
                    ...
                    <div class="form-group">
                        ...
                        <select name="campo_pesquisa">
                            ...
                            <option value="renavam" <?= $campo_pesquisa == 'renavam' ? 'selected' : '' ?>>Renavam</option>
                            <option value="chassi" <?= $campo_pesquisa == 'chassi' ? 'selected' : '' ?>>Chassi</option>
                            <option value="proprietario" <?= $campo_pesquisa == 'proprietario' ? 'selected' : '' ?>>Proprietário</option>
                        </select>
                    </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php if(empty($veiculos)): ?>
                            <tr>
                                <td colspan="7" style="text-align: center;">Nenhum veículo encontrado para estes filtros.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach($veiculos as $veiculo): ?>
                                <tr>
                                    <td class="col-data"><?= $veiculo['VEI_DT_CADASTRO'] ? date('d/m/Y', strtotime($veiculo['VEI_DT_CADASTRO'])) : 'N/A' ?></td>
                                    <td><?= htmlspecialchars($veiculo['PLACA_UF'] ?? 'N/A') ?></td>
                                    <td><?= htmlspecialchars($veiculo['MODELO'] ?? 'N/A') ?></td>
                                    <td><?= htmlspecialchars($veiculo['ANO_FABRI'] ?? 'N/A') ?></td>
                                    <td><?= htmlspecialchars($veiculo['ANO'] ?? 'N/A') ?></td>
                                    <td><?= htmlspecialchars($veiculo['RENAVAM'] ?? 'N/A') ?></td>
                                    <td class="col-prop"><?= htmlspecialchars($veiculo['CLIENTE_NOME'] ?? 'N/A') ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
            ...
            <div class="pagination">
                <span class="span_paginacao">Página <?= $page ?> de <?= $total_pages ?> (Total de <?= $total_records ?> veículos)</span>
            </div>
