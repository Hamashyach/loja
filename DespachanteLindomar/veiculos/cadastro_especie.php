...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Espécie salvo com sucesso.';
            $message_type = 'success';
            break;
        case 'deleted':
            $message = 'Espécie excluído com sucesso.';
            $message_type = 'success';
            break;
        case 'error':
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_especie';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_especie';
    $stmt = $conn->prepare("SELECT * FROM ESPECIE WHERE COD_ESPE = ?");
    $stmt->execute([$id]);
    $especie_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM ESPECIE WHERE COD_ESPE = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_especie.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_especie.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['COD_ESPE'] ?: null;   
    $dados = [
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO ESPECIE ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE ESPECIE SET " . implode(', ', $set_sql) . " WHERE COD_ESPE = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_especie.php?status=success");
        exit;
    } catch (PDOException $e) {
        die("Erro ao salvar tipo de especie: " . $e->getMessage());
    }
}

// --- LÓGICA DE BUSCA E CONSULTA ---
$sql_base = "SELECT * FROM ESPECIE";
$sql_count_base = "SELECT COUNT(*) FROM ESPECIE";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "COD_ESPE LIKE ?";
...
$sql_final = $sql_base ." ORDER BY COD_ESPE LIMIT $limit OFFSET $offset";
...
$stmt->execute($params);
$especie = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Lógica  de navegação
$query_string = http_build_query(array_merge($_GET, ['page' => '']));
$link_primeiro = "cadastro_especie.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_especie.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_especie.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_especie.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Cadastro de Especie - Lindomar Despachante</title>
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php endif; ?>
             ...
             <?php 
                if (($action === 'edit') && !empty($especie_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong> <?= htmlspecialchars($especie_para_editar['COD_ESP'] ?? '')?><br><br>
                        DESCRIÇÃO: <?= htmlspecialchars($especie_para_editar['DESCRICAO']?? '') ?>
                    </div>

                    <div class="client-actions">
                        <a href="cadastro_especie.php" class="btn btn-danger">Voltar para Consulta</a>
                    </div>
                </div>
            ...
            <?php else :?>
                <div class="form-toolbar">
                    <?php if (temPermissao('gerenciar_config')): ?>
                        <a href="cadastro_especie.php?action=new" class="btn">Novo</a>
                    <?php endif; ?>        
                </div>
            <?php endif; ?>
             ...
             <div class="tabs">
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?= $aba_ativa == 'consulta_especie' ? 'active' : '' ?>" data-tab="consulta_especie" <?php if ($aba_ativa == 'cadastro_especie'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_especie' ? 'active' : '' ?>" data-tab="cadastro_especie"<?= $aba_ativa != 'cadastro_especie' ?'disabled': '' ?>>Cadastro</button>
                </div>

                <div class="tab-content">                
                <div id="consulta_especie" class="tab-pane <?= $aba_ativa == 'consulta_especie' ? 'active' : '' ?>">
                    <form method="GET" action="cadastro_especie.php">
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (!empty($especie)): ?>
                                    <?php foreach ($especie as $especies): ?>
                                    <tr class="clickable-row" data-id="<?= $especies['COD_ESPE'] ?>">
                                        <td><?= htmlspecialchars($especies['COD_ESPE']??'') ?></td>
                                        <td><?= htmlspecialchars($especies['DESCRICAO']??'') ?></td>
                                        <td><?= htmlspecialchars($especies['ABREVIATURA']??'') ?></td>
                                        <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                            <?php if (temPermissao('gerenciar_config')) : ?>
                                                <a href="cadastro_especie.php?action=edit&id=<?=$especies['COD_ESPE'] ?>" class="btn btn-primary">Editar</a>
                                            <?php endif; ?>
                                            <?php if (temPermissao('gerenciar_config')) : ?>
                                                <a href="cadastro_especie.php?action=delete&id=<?=$especies['COD_ESPE'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                            <?php endif; ?>
                                        </td>      
                                    </tr>
                                    <?php endforeach; ?>
               ...
               <div id="cadastro_especie" class="tab-pane <?= $aba_ativa == 'cadastro_especie' ? 'active' : '' ?>">
                    <form id="formEspecie" method="POST" action="cadastro_especie.php">
                        <input type="hidden" name="COD_ESPE" value="<?= htmlspecialchars($especie_para_editar['COD_ESPE'] ?? '') ?>">
                            ...
                            <fieldset class="form-section">
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-group" style="max-width: 100px;">
                                        <label for="COD_ESPE">Código</label>
                                        <input type="text" id="COD_ESPE" value="<?= htmlspecialchars($especie_para_editar['COD_ESPE'] ?? 'NOVO') ?>" readonly>
                                    </div>

                                    <div class="form-group">
                                        <label for="DESCRICAO">Descrição</label>
                                        <input type="text" id="DESCRICAO" name="DESCRICAO" value="<?= htmlspecialchars($especie_para_editar['DESCRICAO'] ?? '') ?>" required>
                                    </div>

                                    <div class="form-group">
                                        <label for="ABREVIATURA">Abreviatura</label>
                                        <input type="text" id="ABREVIATURA" name="ABREVIATURA" value="<?= htmlspecialchars($especie_para_editar['ABREVIATURA'] ?? '') ?>" required>
                                    </div>
                                </div>
                            </fieldset>
                        </fieldset>
                        <div class="form-footer">
                            ...
                            <?php endif; ?>
                            <a href="cadastro_especie.php" class="btn btn-danger">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
