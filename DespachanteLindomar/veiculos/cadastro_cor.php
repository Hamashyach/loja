...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Cor salvo com sucesso.';
            $message_type = 'success';
            break;
        case 'deleted':
            $message = 'Cor excluído com sucesso.';
            $message_type = 'success';
            break;
        case 'error':
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_cor';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_cor';
    $stmt = $conn->prepare("SELECT * FROM COR WHERE COD_COR = ?");
    $stmt->execute([$id]);
    $cor_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM COR WHERE COD_COR = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_cor.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_cor.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['COD_COR'] ?: null;   
    $dados = [
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO COR ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE COR SET " . implode(', ', $set_sql) . " WHERE COD_COR = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_cor.php?status=success");
        exit;
    } catch (PDOException $e) {
        die("Erro ao salvar tipo de cor: " . $e->getMessage());
    }
}

// --- LÓGICA DE BUSCA E CONSULTA ---
$sql_base = "SELECT * FROM COR";
$sql_count_base = "SELECT COUNT(*) FROM COR";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "COD_COR LIKE ?";
...
$sql_final = $sql_base ." ORDER BY COD_COR LIMIT $limit OFFSET $offset";
...
$link_primeiro = "cadastro_cor.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_cor.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_cor.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_cor.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Cadastro de Cor - Lindomar Despachante</title>
  ...
  <header class="top-menu">
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php 
                if (($action === 'edit') && !empty($cor_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong> <?= htmlspecialchars($cor_para_editar['COD_COR'] ?? '')?><br><br>
                        DESCRIÇÃO: <?= htmlspecialchars($cor_para_editar['DESCRICAO']?? '') ?>
                    </div>

                    <div class="client-actions">
                        <a href="cadastro_cor.php" class="btn btn-danger">Voltar para Consulta</a>
                    </div>
                </div>
            ...
            <?php else :?>
                <div class="form-toolbar">
                    <?php if (temPermissao('gerenciar_config')): ?>
                        <a href="cadastro_cor.php?action=new" class="btn">Novo</a>
                    <?php endif; ?>        
                </div>
            <?php endif; ?>
             ...
             <div class="tabs">
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?= $aba_ativa == 'consulta_cor' ? 'active' : '' ?>" data-tab="consulta_cor" <?php if ($aba_ativa == 'cadastro_cor'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_cor' ? 'active' : '' ?>" data-tab="cadastro_cor"<?= $aba_ativa !== 'cadastro_cor' ? 'disabled' : '' ?>>Cadastro</button>
                </div>

                <div class="tab-content">                
                <div id="consulta_cor" class="tab-pane <?= $aba_ativa == 'consulta_cor' ? 'active' : '' ?>" style="padding: 20px;">
                    <form method="GET" action="cadastro_cor.php">
                    ...
                    <div class="table-container">
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (!empty($cor)): ?>
                                    <?php foreach ($cor as $cors): ?>
                                    <tr class="clickable-row" data-id="<?= $cors['COD_COR'] ?>">
                                        <td><?= htmlspecialchars($cors['COD_COR']??'') ?></td>
                                        <td><?= htmlspecialchars($cors['DESCRICAO']??'') ?></td>   
                                        <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                            <?php if (temPermissao('gerenciar_config')) : ?>
                                                <a href="cadastro_cor.php?action=edit&id=<?=$cors['COD_COR'] ?>" class="btn btn-primary">Editar</a>
                                            <?php endif; ?>
                                            <?php if (temPermissao('gerenciar_config')) : ?>
                                                <a href="cadastro_cor.php?action=delete&id=<?=$cors['COD_COR'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                            <?php endif; ?>
                                        </td>         
                                    </tr>
                                    <?php endforeach; ?>
               ...
               <div id="cadastro_cor" class="tab-pane <?= $aba_ativa == 'cadastro_cor' ? 'active' : '' ?>">
                    <form id="formCor" method="POST" action="cadastro_cor.php">
                        <input type="hidden" name="COD_COR" value="<?= htmlspecialchars($cor_para_editar['COD_COR'] ?? '') ?>">
                                ...
                                <fieldset class="form-section">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group" style="max-width: 100px;">
                                            ...
                                            <input type="text" id="COD_COR"  value="<?= htmlspecialchars($cor_para_editar['COD_COR'] ?? 'NOVO') ?>" readonly>
                                        </div>
                                        <div class="form-group">
                                            <label for="DESCRICAO">Descrição</label>
                                            <input type="text" id="DESCRICAO" name="DESCRICAO" value="<?= htmlspecialchars($cor_para_editar['DESCRICAO'] ?? '') ?>">
                                        </div>
                                    </div>
                                </fieldset>
                            </fieldset>
                            <div class="form-footer">
                                ...
                                <?php endif; ?>
                                <a href="cadastro_cor.php" class="btn btn-danger">Cancelar</a>
                            </div>
                    </form>
                </div>
            </div>
