...
$aba_ativa = 'consulta_tempos_licenciamento';
$tempos_licenciamento_para_editar = [];
$tempos_licenciamento = [];
...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Tempos de Licenciamento salvo com sucesso.';
            $message_type = 'success';
            break;
        case 'deleted':
            $message = 'Tempos de Licenciamento excluído com sucesso.';
            $message_type = 'success';
            break;
        case 'error':
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_tempos_licenciamento';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_tempos_licenciamento';
    $stmt = $conn->prepare("SELECT * FROM LICENCIAMENTOS_TEMPOS WHERE LIC_CODIGO = ?");
    $stmt->execute([$id]);
    $tempos_licenciamento_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM LICENCIAMENTOS_TEMPOS WHERE LIC_CODIGO = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_tempos_licenciamento.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_tempos_licenciamento.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['LIC_CODIGO'] ?: null;   
    $dados = [
        'LIC_FINAL' => $_POST['LIC_FINAL'] ?? null,
        'LIC_TEMPODESC' => $_POST['LIC_TEMPODESC'] ?? null
    ];
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO LICENCIAMENTOS_TEMPOS ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE LICENCIAMENTOS_TEMPOS SET " . implode(', ', $set_sql) . " WHERE LIC_CODIGO = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_tempos_licenciamento.php?status=success");
        exit;
    } catch (PDOException $e) {
...
$sql_base = "SELECT * FROM LICENCIAMENTOS_TEMPOS";
$sql_count_base = "SELECT COUNT(*) FROM LICENCIAMENTOS_TEMPOS";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "LIC_CODIGO LIKE ?";
    elseif ($campo == 'lic_final') $where_clauses[] = "LIC_FINAL LIKE ?";
    elseif ($campo == 'mes') $where_clauses[] = "LIC_TEMPODESC LIKE ?";
...
$sql_final = $sql_base ." ORDER BY LIC_CODIGO LIMIT $limit OFFSET $offset";
...
$stmt->execute($params);
$tempos_licenciamento = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Lógica  de navegação
$query_string = http_build_query(array_merge($_GET, ['page' => '']));
$link_primeiro = "cadastro_tempos_licenciamento.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_tempos_licenciamento.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_tempos_licenciamento.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_tempos_licenciamento.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Tempos de Licenciamento - Lindomar Despachante</title>
...
<body>
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php 
                if (($action === 'edit') && !empty($tempos_licenciamento_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong><?= htmlspecialchars($tempos_licenciamento_para_editar['LIC_CODIGO'] ?? '')?><br><br>
                        FINAL DA PLACA: <?= htmlspecialchars($tempos_licenciamento_para_editar['LIC_FINAL']?? '') ?> <br><br>
                        MÊS DE VENCIMENTO: <?= htmlspecialchars($tempos_licenciamento_para_editar['LIC_TEMPODESC']?? '') ?>
                    </div>
            ...
            <div class="tabs">
                ...
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?=$aba_ativa == 'consulta_tempos_licenciamento' ? 'active' : '' ?>" data-tab="consulta_tempos_licenciamento" <?php if ($aba_ativa == 'cadastro_tempos_licenciamento'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_tempos_licenciamento' ? 'active' : '' ?>" data-tab="cadastro_tempos_licenciamento" <?= $aba_ativa != 'cadastro_tempos_licenciamento' ? 'disabled' : '' ?>>Cadastro</button>
                </div>

                <div class="tab-content">
                    <!-- Consulta -->
                    <div id="consulta_tempos_licenciamento" class="tab-pane <?=$aba_ativa == 'consulta_tempos_licenciamento' ? 'active' : '' ?>" style="padding: 20px;">
                        <form method="GET" action="cadastro_tempos_licenciamento.php">
                            <fieldset class="search-box">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="campo_pesquisa" name="campo_pesquisa">
                                        ...
                                        <option value="lic_final" <?= ($_GET['campo_pesquisa'] ?? '') === 'lic_final' ? 'selected' : '' ?>>FINAL PLACA</option>
                                        <option value="mes" <?= ($_GET['campo_pesquisa'] ?? '') === 'mes' ? 'selected' : '' ?>>MÊS VENCIMENTO</option>
                                    </select>
                                </div>
                        ...
                        <div class="table-container">
                            ...
                            <table class="data-table">
                                ...
                                <tbody>
                                    <?php if (!empty($tempos_licenciamento)): ?>
                                        <?php foreach ($tempos_licenciamento as $tempos_licenciamentos): ?>
                                            <tr class="clickable-row" data-id="<?= htmlspecialchars($tempos_licenciamentos['LIC_CODIGO'] ?? '') ?>">
                                                <td><?= htmlspecialchars($tempos_licenciamentos['LIC_CODIGO'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($tempos_licenciamentos['LIC_FINAL'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($tempos_licenciamentos['LIC_TEMPODESC'] ?? '') ?></td>
                                                <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_tempos_licenciamento.php?action=edit&id=<?=$tempos_licenciamentos['LIC_CODIGO'] ?>" class="btn btn-primary">Editar</a>
                                                <?php endif; ?>
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_tempos_licenciamento.php?action=delete&id=<?=$tempos_licenciamentos['LIC_CODIGO'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                                <?php endif; ?>
                                            </td>        
                                            </tr>
                                        <?php endforeach; ?>
                    ...
                    <!--Cadastro-->
                    <div id="cadastro_tempos_licenciamento" class="tab-pane <?=$aba_ativa == 'cadastro_tempos_licenciamento' ? 'active' : '' ?>">
                        <form id="formTempoLicenciamento" method="POST" action="cadastro_tempos_licenciamento.php">
                            <input type="hidden" name="LIC_CODIGO" value="<?= htmlspecialchars($tempos_licenciamento_para_editar['LIC_CODIGO'] ?? '') ?>">
                            <fieldset <?= ($action === 'view') ? 'disabled' : '' ?> style="border: none;">
                                <fieldset class="form-section">
                                    <legend>Dados do Licenciamento</legend>
                                    <div class="form-row">
                                        <div class="form-group">
                                            <label for="LIC_CODIGO">Código</label>
                                            <input type="text" id="LIC_CODIGO" value="<?= htmlspecialchars($tempos_licenciamento_para_editar['LIC_CODIGO'] ?? 'NOVO') ?>" readonly>
                                        </div>

                                        <div class="form-group">
                                            <label for="LIC_FINAL">Final da Placa</label>
                                            <input type="text" id="LIC_FINAL" name="LIC_FINAL" min="0" max="9" value="<?=htmlspecialchars($tempos_licenciamento_para_editar['LIC_FINAL'] ?? '')?>" required>
                                        </div>
                                        <div class="form-group" style="flex-grow: 2;">
                                            <label for="LIC_TEMPODESC">Mês de Vencimento</label>
                                            <input type="text" id="LIC_TEMPODESC" name="LIC_TEMPODESC" value="<?=htmlspecialchars($tempos_licenciamento_para_editar['LIC_TEMPODESC'] ?? '')?>" required>
                                        </div>
                                    </div>
                                </fieldset>
                            </fieldset>
                            ...
                            <div class="form-footer">
                                ...
                                <?php endif; ?>
                                <a href="cadastro_tempos_licenciamento.php" class="btn btn-danger">Cancelar</a>
                            </div>
                        </form>
                    </div>
                </div>
