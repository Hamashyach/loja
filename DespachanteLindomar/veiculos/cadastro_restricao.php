...
$aba_ativa = 'consulta_restricao';
$restricao_para_editar = [];
$restricao = [];
...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Restrição salva com sucesso.';
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_restricao';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_restricao';
    $stmt = $conn->prepare("SELECT * FROM RESTRICOES WHERE COD_REST = ?");
    $stmt->execute([$id]);
    $restricao_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM RESTRICOES WHERE COD_REST = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_restricao.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_restricao.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['COD_REST'] ?: null;   
    $dados = [
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO RESTRICOES ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE RESTRICOES SET " . implode(', ', $set_sql) . " WHERE COD_REST = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_restricao.php?status=success");
        exit;
    } catch (PDOException $e) {
        die("Erro ao salvar restrição: " . $e->getMessage());
    }
}

// --- LÓGICA DE BUSCA E CONSULTA ---
$sql_base = "SELECT * FROM RESTRICOES";
$sql_count_base = "SELECT COUNT(*) FROM RESTRICOES";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "COD_REST LIKE ?";
...
$sql_final = $sql_base ." ORDER BY COD_REST LIMIT $limit OFFSET $offset";
...
$stmt->execute($params);
$restricao = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Lógica  de navegação
$query_string = http_build_query(array_merge($_GET, ['page' => '']));
$link_primeiro = "cadastro_restricao.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_restricao.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_restricao.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_restricao.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Cadastro de Restrição - Lindomar Despachante</title>
...
<body>
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php 
                if (($action === 'edit') && !empty($restricao_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong> <?= htmlspecialchars($restricao_para_editar['COD_REST'] ?? '')?><br><br>
                        DESCRIÇÃO: <?= htmlspecialchars($restricao_para_editar['DESCRICAO']?? '') ?>
                    </div>

                    <div class="client-actions">
                        <a href="cadastro_restricao.php" class="btn btn-danger">Voltar para Consulta</a>
                    </div>
                </div>
            ...
            <?php else :?>
                <div class="form-toolbar">
                    <?php if (temPermissao('gerenciar_config')): ?>
                        <a href="cadastro_restricao.php?action=new" class="btn">Novo</a>
                    <?php endif; ?>        
                </div>
            <?php endif; ?>
             ...
             <div class="tabs">
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?= $aba_ativa == 'consulta_restricao' ? 'active' : '' ?>" data-tab="consulta_restricao" <?php if ($aba_ativa == 'cadastro_restricao'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_restricao' ? 'active' : '' ?>" data-tab="cadastro_restricao"<?= $aba_ativa !== 'cadastro_restricao' ? 'disabled' : '' ?>>Cadastro</button>
                </div>

                <div class="tab-content">                
                <div id="consulta_restricao" class="tab-pane <?= $aba_ativa == 'consulta_restricao' ? 'active' : '' ?>" style="padding: 20px;">
                        <form method="GET" action="cadastro_restricao.php">
                        ...
                        <div class="table-container">
                            ...
                            <table class="data-table">
                                ...
                                <tbody>
                                    <?php if (!empty($restricao)): ?>
                                        <?php foreach ($restricao as $restricoes): ?>
                                            <tr class="clickable-row" data-id="<?= $restricoes['COD_REST'] ?>">
                                                <td><?= htmlspecialchars($restricoes['COD_REST']??'') ?></td>
                                                <td><?= htmlspecialchars($restricoes['DESCRICAO']??'') ?></td>
                                                <td><?= htmlspecialchars($restricoes['ABREVIATURA']??'') ?></td>
                                                <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_restricao.php?action=edit&id=<?=$restricoes['COD_REST'] ?>" class="btn btn-primary">Editar</a>
                                                <?php endif; ?>
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_restricao.php?action=delete&id=<?=$restricoes['COD_REST'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                                <?php endif; ?>
                                            </td>    
                                            </tr>
                                        <?php endforeach; ?>
                    ...
                    <!--cadastro-->              
                    <div id="cadastro_restricao" class="tab-pane <?= $aba_ativa == 'cadastro_restricao' ? 'active' : '' ?>">
                        <form id="formrestricao" method="POST" action="cadastro_restricao.php">
                            <input type="hidden" name="COD_REST" value="<?= htmlspecialchars($restricao_para_editar['COD_REST'] ?? '') ?>"> 
                            <fieldset <?= ($action === 'view') ? 'disabled' : '' ?> style="border: none;"> 
                                <fieldset class="form-section">
                                    <legend>Dados Gerais</legend>
                                    <div class="form-row">
                                        <div class="form-group" style="max-width: 100px;">
                                            <label for="COD_REST">Código</label>
                                            <input type="text"  id="COD_REST" value="<?= htmlspecialchars($restricao_para_editar['COD_REST'] ?? 'NOVO') ?>" readonly>
                                        </div>
                                        <div class="form-group">
                                            <label for="DESCRICAO">Descrição</label>
                                            <input type="text" id="DESCRICAO" name="DESCRICAO" value="<?= htmlspecialchars($restricao_para_editar['DESCRICAO'] ?? '') ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="ABREVIATURA">Abreviação</label>
                                            <input type="text" id="ABREVIATURA" name="ABREVIATURA" value="<?= htmlspecialchars($restricao_para_editar['ABREVIATURA'] ?? '') ?>">
                                        </div>
                                    </div>
                                </fieldset>
                            </fieldset>
                            <div class="form-footer">
                                ...
                                <?php endif; ?>
                                <a href="cadastro_restricao.php" class="btn btn-danger">Cancelar</a>
                            </div>
                      </form>
                </div>
