...
$aba_ativa = 'consulta_carroceria';
$carroceria_para_editar = [];
$carroceria = [];
...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Carroceria salvo com sucesso.';
            $message_type = 'success';
            break;
        case 'deleted':
            $message = 'Carroceria excluído com sucesso.';
            $message_type = 'success';
            break;
        case 'error':
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_carroceria';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_carroceria';
    $stmt = $conn->prepare("SELECT * FROM CARROCERIA WHERE COD_CARR = ?");
    $stmt->execute([$id]);
    $carroceria_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM CARROCERIA WHERE COD_CARR = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_carroceria.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_carroceria.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['COD_CARR'] ?: null;   
    $dados = [
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO CARROCERIA ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE CARROCERIA SET " . implode(', ', $set_sql) . " WHERE COD_CARR = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_carroceria.php?status=success");
        exit;
    } catch (PDOException $e) {
        die("Erro ao salvar tipo de carroceria: " . $e->getMessage());
    }
}

// --- LÓGICA DE BUSCA E CONSULTA ---
$sql_base = "SELECT * FROM CARROCERIA";
$sql_count_base = "SELECT COUNT(*) FROM CARROCERIA";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "COD_CARR ?";
...
$sql_final = $sql_base ." ORDER BY COD_CARR LIMIT $limit OFFSET $offset";
...
$stmt->execute($params);
$carroceria = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Lógica  de navegação
$query_string = http_build_query(array_merge($_GET, ['page' => '']));
$link_primeiro = "cadastro_carroceria.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_carroceria.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_carroceria.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_carroceria.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Cadastro de Carroceria - Lindomar Despachante</title>
...
<body>
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php 
                if (($action === 'edit') && !empty($carroceria_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong> <?= htmlspecialchars($carroceria_para_editar['COD_CARR'] ?? '')?><br><br>
                        DESCRIÇÃO: <?= htmlspecialchars($carroceria_para_editar['DESCRICAO']?? '') ?>
                    </div>
            ...
            <?php endif; ?>
             ...
             <div class="tabs">
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?=$aba_ativa == 'consulta_carroceria' ? 'active' : '' ?>" data-tab="consulta_carroceria" <?php if ($aba_ativa == 'cadastro_carroceria'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_carroceria' ? 'active' : '' ?>" data-tab="cadastro_carroceria" <?= $aba_ativa != 'cadastro_carroceria' ? 'disabled' : '' ?>>Cadastro</button>
                </div>

                <div class="tab-content"> 
                <!--consulta-->               
                <div id="consulta_carroceria" class="tab-pane <?= $aba_ativa == 'consulta_carroceria' ? 'active' : '' ?>" style="padding: 20px;">
                    <form method="GET" action="cadastro_carroceria.php">
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (!empty($carroceria)): ?>
                                    <?php foreach ($carroceria as $carrocerias): ?>
                                        <tr class="clickable-row" data-id="<?= $carrocerias['COD_CARR'] ?>">
                                            <td><?= htmlspecialchars($carrocerias['COD_CARR']??'') ?></td>
                                            <td><?= htmlspecialchars($carrocerias['DESCRICAO']??'') ?></td>
                                            <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_carroceria.php?action=edit&id=<?=$carrocerias['COD_CARR'] ?>" class="btn btn-primary">Editar</a>
                                                <?php endif; ?>
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_carroceria.php?action=delete&id=<?=$carrocerias['COD_CARR'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                                <?php endif; ?>
                                            </td>                               
                                        </tr>
                                    <?php endforeach; ?>
                ...
                <div style="display: flex; justify-content: space-between; margin-top: 30px;;" class="pagination">
                  ...
                  <!--cadastro--> 
                <div id="cadastro_carroceria" class="tab-pane <?= $aba_ativa == 'cadastro_carroceria' ? 'active' : '' ?>">
                        <form id="formCarroceria" method="POST" action="cadastro_carroceria.php">
                            <input type="hidden" name="COD_CARR" value="<?= htmlspecialchars($carroceria_para_editar['COD_CARR'] ?? '') ?>">
                            <fieldset <?= ($action === 'view') ? 'disabled' : '' ?> style="border: none;">
                                <fieldset class="form-section">
                                    <legend>Dados Gerais</legend>
                                    <div class="form-row">
                                        <div class="form-group" style="max-width: 100px;">
                                            <label for="COD_CARR">Código</label>
                                            <input type="text" id="COD_CARR" value="<?= htmlspecialchars($carroceria_para_editar['COD_CARR'] ?? 'NOVO') ?>" readonly>
                                        </div>

                                        <div class="form-group">
                                            <label for="DESCRICAO">Descrição</label>
                                            <input type="text" id="DESCRICAO" name="DESCRICAO" value="<?= htmlspecialchars($carroceria_para_editar['DESCRICAO'] ?? '') ?>" required>
                                        </div>
                                    </div>                                   
                                </fieldset>
                            </fieldset>
                            <div class="form-footer">
                                ...
                                <?php endif; ?>
                                <a href="cadastro_carroceria.php" class="btn btn-danger">Cancelar</a>
                            </div>
                        </form>
                </div>
            </div>
