...
protegerPagina('acessar_municipios');

require_once '../config.php'; 

// INICIALIZAÇÃO E PAGINAÇÃO
$aba_ativa = 'consulta_municipio';
$municipio_para_editar = [];
$municipios = [];
...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Município salvo com sucesso.';
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_municipio';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_municipio';
    $stmt = $conn->prepare("SELECT * FROM MUNICIPIO WHERE CODIGO = ?");
    $stmt->execute([$id]);
    $municipio_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM MUNICIPIO WHERE CODIGO = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_municipios.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_municipios.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO MUNICIPIO ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE MUNICIPIO SET " . implode(', ', $set_sql) . " WHERE CODIGO = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_municipios.php?status=success");
        exit;
    } catch (PDOException $e) {
        die("Erro ao salvar município: " . $e->getMessage());
    }
}


// --- LÓGICA DE BUSCA E CONSULTA ---
$sql_base = "SELECT * FROM MUNICIPIO";
$sql_count_base = "SELECT COUNT(*) FROM MUNICIPIO";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    elseif ($campo == 'cod_muni') $where_clauses[] = "COD_MUNI LIKE ?";
    elseif ($campo == 'municipio') $where_clauses[] = " MUNICIPIO LIKE ?";
    elseif ($campo == 'estado') $where_clauses[] = " ESTADO LIKE ?";
...
$sql_final = $sql_base ." ORDER BY MUNICIPIO LIMIT $limit OFFSET $offset";
...
$link_primeiro = "cadastro_municipios.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_municipios.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_municipios.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_municipios.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Municípios - Lindomar Despachante</title>
...
<body>
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php 
                if (($action === 'edit') && !empty($municipio_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>MUNICÍPIO:</strong> <?= htmlspecialchars($municipio_para_editar['MUNICIPIO'] ?? '')?><br><br>
                        CEP: <?= htmlspecialchars($municipio_para_editar['CEP']?? '') ?>
                    </div>

                    <div class="client-actions">
                        <a href="cadastro_municipios.php" class="btn btn-danger">Voltar para Consulta</a>
                    </div>
                </div>
            ...
            <?php else :?>
                <div class="form-toolbar">
                    <?php if (temPermissao('gerenciar_config')): ?>
                        <a href="cadastro_municipios.php?action=new" class="btn">Novo</a>
                    <?php endif; ?>        
                </div>
            <?php endif; ?>
            ...
            <div class="tabs">
                ...
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?=$aba_ativa == 'consulta_municipio' ? 'active' : '' ?>" data-tab="consulta_municipio" <?php if ($aba_ativa == 'cadastro_municipio'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_municipio' ? 'active' : '' ?>" data-tab="cadastro_municipio" <?= $aba_ativa != 'cadastro_municipio' ? 'disabled' : '' ?>>Cadastro</button>
                </div>
                ...
                <div class="tab-content">
                    ...
                    <!--Consulta-->
                    <div id="consulta_municipio" class="tab-pane <?= $aba_ativa == 'consulta_municipio' ? 'active' : '' ?>">
                        <form method="GET" action="cadastro_municipios.php">
                            <fieldset class="search-box">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="campo_pesquisa" name="campo_pesquisa">
                                        ...
                                        <option value="cod_muni"<?= ($_GET['campo_pesquisa'] ?? '') === 'cod_muni' ? 'selected' : '' ?>>CÓDIGO DE MUNICÍPIO</option>
                                        <option value="municipio"<?= ($_GET['campo_pesquisa'] ?? '') === 'municipio' ? 'selected' : '' ?>>MUNICÍPIO</option>
                                        <option value="estado"<?= ($_GET['campo_pesquisa'] ?? '') === 'estado' ? 'selected' : '' ?>>ESTADO</option>
                                    </select>
                                </div>
                            ...
                            <table class="data-table">
                                ...
                                <tbody>
                                    <?php if (!empty($municipios)): ?>
                                        ...
                                        <tr class="clickable-row" data-id="<?= $municipio['CODIGO'] ?>">
                                            <td><?=htmlspecialchars($municipio['CODIGO'] ?? '')?></td>
                                            <td><?=htmlspecialchars($municipio['COD_MUNI'] ?? '')?></td>
                                            <td><?=htmlspecialchars($municipio['MUNICIPIO'] ?? '')?></td>
                                            <td><?=htmlspecialchars($municipio['ESTADO'] ?? '')?></td>
                                            <td><?=htmlspecialchars($municipio['CEP'] ?? '')?></td>
                                            <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_municipios.php?action=edit&id=<?=$municipio['CODIGO'] ?>" class="btn btn-primary">Editar</a>
                                                <?php endif; ?>
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_municipios.php?action=delete&id=<?=$municipio['CODIGO'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                                <?php endif; ?>
                                            </td>            
                                        </tr>
                                        <?php endforeach; ?>
                    ...
                    <!--Cadastro-->
                    <div id="cadastro_municipio" class="tab-pane <?=$aba_ativa == 'cadastro_municipio' ? 'active' : '' ?>">
                        <form id="formmunicipio" method="POST" action="cadastro_municipios.php">
                            <input type="hidden" name="CODIGO" value="<?= htmlspecialchars($municipio_para_editar['CODIGO'] ?? '') ?>">
                                ...
                                <fieldset class="form-section">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group" style="max-width: 100px;">
                                            <label>Código</label>
                                            <input type="text" value="<?= htmlspecialchars($municipio_para_editar['CODIGO'] ?? 'NOVO') ?>" readonly>
                                        </div>
                                        <div class="form-group" style="max-width: 100px;">
                                            <label>Código Município</label>
                                            <input type="text" id="COD_MUNI" name="COD_MUNI" value="<?= htmlspecialchars($municipio_para_editar['COD_MUNI'] ?? '') ?>"required> 
                                        </div>
                                        <div class="form-group" style="flex-grow: 3;">
                                            <label for="MUNICIPIO">Município</label>
                                            <input type="text" id="MUNICIPIO" name="MUNICIPIO" value="<?= htmlspecialchars($municipio_para_editar['MUNICIPIO'] ?? '') ?>"required> 
                                        </div>
                                        <div class="form-group" style="max-width: 150px;">
                                            <label for="CEP">CEP</label>
                                            <input type="text" id="CEP" name="CEP" value="<?= htmlspecialchars($municipio_para_editar['CEP'] ?? '') ?>">
                                        </div>
                                        <div class="form-group" style="max-width: 80px;">
                                            <label for="ESTADO">UF</label>
                                            <input type="text" id="ESTADO" name="ESTADO" value="<?= htmlspecialchars($municipio_para_editar['ESTADO'] ?? '') ?>" >   
                            ...
                            <div class="form-footer">
                                ...
                                <?php endif; ?>
                                <a href="cadastro_municipios.php" class="btn btn-danger">Cancelar</a>
                            </div>
                        </form>
                    </div>
                </div>
