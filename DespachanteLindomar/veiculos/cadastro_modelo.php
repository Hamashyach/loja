...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Cadastro salvo com sucesso.';
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_modelo';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_modelo';
    $stmt = $conn->prepare("SELECT * FROM MARCA WHERE COD_MARCA = ?");
    $stmt->execute([$id]);
    $modelo_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM MARCA WHERE COD_MARCA = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_modelo.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_modelo.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['COD_MARCA'] ?: null;   
    $dados = [
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO MARCA ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE MARCA SET " . implode(', ', $set_sql) . " WHERE COD_MARCA = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_modelo.php?status=success");
        exit;
    } catch (PDOException $e) {
...
$sql_base = "SELECT * FROM MARCA";
$sql_count_base = "SELECT COUNT(*) FROM MARCA";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "COD_MARCA LIKE ?";
...
$sql_final = $sql_base ." ORDER BY COD_MARCA LIMIT $limit OFFSET $offset";
...
$link_primeiro = "cadastro_modelo.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_modelo.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_modelo.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_modelo.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Cadastro de Marca/Modelo - Lindomar Despachante</title>
   ...
   <header class="top-menu">
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php 
                if (($action === 'edit') && !empty($modelo_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong> <?= htmlspecialchars($modelo_para_editar['COD_MARCA'] ?? '')?><br><br>
                        DESCRIÇÃO: <?= htmlspecialchars($modelo_para_editar['DESCRICAO']?? '') ?>
                    </div>

                    <div class="client-actions">
                        <a href="cadastro_modelo.php" class="btn btn-danger">Voltar para Consulta</a>
                    </div>
                </div>
            ...
            <?php else :?>
                <div class="form-toolbar">
                    <?php if (temPermissao('gerenciar_config')): ?>
                        <a href="cadastro_modelo.php?action=new" class="btn">Novo</a>
                    <?php endif; ?>        
                </div>
            <?php endif; ?>
             ...
             <div class="tabs">
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?= $aba_ativa == 'consulta_modelo' ? 'active' : '' ?>" data-tab="consulta_modelo" <?php if ($aba_ativa == 'cadastro_modelo'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_modelo' ? 'active' : '' ?>" data-tab="cadastro_modelo"<?= $aba_ativa != 'cadastro_modelo' ? 'disabled' : '' ?>>Cadastro</button>
                </div>

                <div class="tab-content">
                    
                <!--CONSULTA-->
                <div id="consulta_modelo" class="tab-pane <?= $aba_ativa == 'consulta_modelo' ? 'active' : '' ?>"  style="padding: 20px;">
                    <form method="GET" action="cadastro_modelo.php">
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (!empty($modelo)): ?>
                                    ...
                                    <tr class="clickable-row" data-id="<?= $modelos['COD_MARCA'] ?>">
                                        <td><?= htmlspecialchars($modelos['COD_MARCA']??'') ?></td>
                                        <td><?= htmlspecialchars($modelos['DESCRICAO']??'') ?></td>
                                        <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                            <?php if (temPermissao('gerenciar_config')) : ?>
                                                <a href="cadastro_modelo.php?action=edit&id=<?=$modelos['COD_MARCA'] ?>" class="btn btn-primary">Editar</a>
                                            <?php endif; ?>
                                            <?php if (temPermissao('gerenciar_config')) : ?>
                                                <a href="cadastro_modelo.php?action=delete&id=<?=$modelos['COD_MARCA'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                            <?php endif; ?>
                                        </td>    
                                    </tr>
                                    <?php endforeach; ?>
                ...
                <!--CADASTRO-->
               <div id="cadastro_modelo" class="tab-pane <?= $aba_ativa == 'cadastro_modelo' ? 'active' : '' ?>">
                    <form id="formmodelo" method="POST" action="cadastro_modelo.php">
                        <input type="hidden" name="COD_MARCA" value="<?= htmlspecialchars($modelo_para_editar['COD_MARCA'] ?? '') ?>">
                            ...
                            <fieldset class="form-section">
                                ...
                                <div class="form-row">
                                    <div class="form-group" style="max-width: 100px;">
                                        ...
                                        <input type="text" id="COD_MARCA" value="<?= htmlspecialchars($modelo_para_editar['COD_MARCA'] ?? 'NOVO') ?>" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="DESCRICAO">Descrição</label>
                                        <input type="text" id="DESCRICAO" name="DESCRICAO" value="<?= htmlspecialchars($modelo_para_editar['DESCRICAO'] ?? '') ?>">
                                    </div>
                                </div>
                            </fieldset>
                        </fieldset>
                        <div class="form-footer">
                            ...
                            <?php endif; ?>
                            <a href="cadastro_modelo.php" class="btn btn-danger">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
