...
$aba_ativa = 'consulta_tipo_endereco';
$tipo_endereco_para_editar = [];
$tipo_endereco = [];
...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Endereço salvo com sucesso.';
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_tipo_endereco';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_tipo_endereco';
    $stmt = $conn->prepare("SELECT * FROM TIPO_LOGRADOURO WHERE TLOG_CODIGO = ?");
    $stmt->execute([$id]);
    $tipo_endereco_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM TIPO_LOGRADOURO WHERE TLOG_CODIGO = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_tipo_endereco.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_tipo_endereco.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['TLOG_CODIGO'] ?: null;   
    $dados = [
        'TLOG_DESCRICAO' => $_POST['TLOG_DESCRICAO'] ?? null,
        'TLOG_ABREVIATURA' => $_POST['TLOG_ABREVIATURA'] ?? null
    ];
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO TIPO_LOGRADOURO ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE TIPO_LOGRADOURO SET " . implode(', ', $set_sql) . " WHERE TLOG_CODIGO = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_tipo_endereco.php?status=success");
        exit;
    } catch (PDOException $e) {
        die("Erro ao salvar tipo de endereco: " . $e->getMessage());
    }
}


// --- LÓGICA DE BUSCA E CONSULTA ---
$sql_base = "SELECT * FROM TIPO_LOGRADOURO";
$sql_count_base = "SELECT COUNT(*) FROM TIPO_LOGRADOURO";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "TLOG_CODIGO LIKE ?";
    elseif ($campo == 'descricao') $where_clauses[] = "TLOG_DESCRICAO LIKE ?";
    elseif ($campo == 'abreviatura') $where_clauses[] = "TLOG_ABREVIATURA LIKE ?";
...
$sql_final = $sql_base ." ORDER BY TLOG_CODIGO LIMIT $limit OFFSET $offset";
...
$stmt->execute($params);
$tipo_endereco = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Lógica  de navegação
$query_string = http_build_query(array_merge($_GET, ['page' => '']));
$link_primeiro = "cadastro_tipo_endereco.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_tipo_endereco.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_tipo_endereco.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_tipo_endereco.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Tipos de Logradouro (Endereço)</title>
           ...
           <?php 
            ...
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong><?= htmlspecialchars($tipos_servico_para_editar['TLOG_CODIGO'] ?? '')?><br><br>
                        DESCRIÇÃO: <?= htmlspecialchars($tipos_servico_para_editar['TLOG_DESCRICAO']?? '') ?>
                    </div>

                    <div class="client-actions">
                        <a href="cadastro_tipo_endereco.php" class="btn btn-danger">Voltar para Consulta</a>
                    </div>
                </div>
            ...
            <?php else :?>
                <div class="form-toolbar">
                    <?php if (temPermissao('gerenciar_config')): ?>
                        <a href="cadastro_tipo_endereco.php?action=new" class="btn">Novo</a>
                    <?php endif; ?>        
                </div>
            <?php endif; ?>
            ...
            <div class="tabs">
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?=$aba_ativa == 'consulta_tipo_endereco' ? 'active' : '' ?>" data-tab="consulta_tipo_endereco" <?php if ($aba_ativa == 'cadastro_tipo_endereco'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?=$aba_ativa == 'cadastro_tipo_endereco' ? 'active' : '' ?>" data-tab="cadastro_tipo_endereco" <?=$aba_ativa != 'cadastro_tipo_endereco' ? 'disabled' : ''?>>Cadastro</button>
                </div>

                <!-- Consulta -->
                <div id="consulta_tipo_endereco" class="tab-pane <?= $aba_ativa == 'consulta_tipo_endereco' ? 'active' : '' ?>" style="padding: 20px;">
                    <form method="GET" action="cadastro_tipo_endereco.php">
                        <fieldset class="search-box">
                            ...
                            <div class="form-group" style="flex-grow: 2;">
                                ...
                                <input type="text" id="parametro_pesquisa" name="valor_pesquisa" value="<?= htmlspecialchars($_GET['valor_pesquisa'] ?? '') ?>"></input>
                            </div>
                    ...
                    <div class="table-container">
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (!empty($tipo_endereco)): ?>
                                    <?php foreach ($tipo_endereco as $tipo_enderecos): ?>
                                        <tr class="clickable-row" data-id="<?= htmlspecialchars($tipo_enderecos['TLOG_CODIGO'] ?? '') ?>">
                                            <td><?= htmlspecialchars($tipo_enderecos['TLOG_CODIGO'] ?? '') ?></td>
                                            <td><?= htmlspecialchars($tipo_enderecos['TLOG_DESCRICAO'] ?? '') ?></td>
                                            <td><?= htmlspecialchars($tipo_enderecos['TLOG_ABREVIATURA'] ?? '') ?></td>
                                            <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_tipo_endereco.php?action=edit&id=<?=$tipo_enderecos['TLOG_CODIGO'] ?>" class="btn btn-primary">Editar</a>
                                                <?php endif; ?>
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_tipo_endereco.php?action=delete&id=<?=$tipo_enderecos['TLOG_CODIGO'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                                <?php endif; ?>
                                            </td>     
                                        </tr>
                                    <?php endforeach; ?>
                ...
                <!--cadastro-->
                <div id="cadastro_tipo_endereco" class="tab-pane <?=$aba_ativa == 'cadastro_tipo_endereco' ? 'active' : '' ?>">
                    <form id="formTipoEndereco" method="POST" action="cadastro_tipo_endereco.php">
                        <input type="hidden" name="TLOG_CODIGO" value="<?= htmlspecialchars($tipos_servico_para_editar['TLOG_CODIGO'] ?? '') ?>">
                        <fieldset <?= ($action === 'view') ? 'disabled' : '' ?> style="border: none;">
                            <fieldset class="form-section">
                                <legend>Dados do Logradouro</legend>
                                <div class="form-row">
                                    <div class="form-group" style="max-width: 120px;">
                                        <label for="TLOG_CODIGO">Código</label>
                                        <input type="text" id="TLOG_CODIGO" value="<?= htmlspecialchars($tipo_endereco_para_editar['TLOG_CODIGO'] ?? 'NOVO') ?>" readonly>
                                    </div>
                                    <div class="form-group" style="flex-grow: 2;">
                                        <label for="TLOG_DESCRICAO">Descrição</label>
                                        <input type="text" id="TLOG_DESCRICAO" name="TLOG_DESCRICAO" value="<?= htmlspecialchars($tipo_endereco_para_editar['TLOG_DESCRICAO'] ?? '')?>">
                                    </div>
                                        <div class="form-group">
                                        <label for="TLOG_ABREVIATURA">Abreviatura</label>
                                        <input type="text" id="TLOG_ABREVIATURA" name="TLOG_ABREVIATURA" value="<?= htmlspecialchars($tipo_endereco_para_editar['TLOG_ABREVIATURA'] ?? '')?>" required>
                                    </div>
                                </div>
                            </fieldset>
                        </fieldset>
                        ...
                        <div class="form-footer">
                            ...
                            <?php endif; ?>
                            <a href="cadastro_tipo_endereco.php" class="btn btn-danger">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
