...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Categoria salvo com sucesso.';
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_categoria';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_categoria';
    $stmt = $conn->prepare("SELECT * FROM CATEGORIA WHERE COD_CATE = ?");
    $stmt->execute([$id]);
...
if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM CATEGORIA WHERE COD_CATE = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_categoria.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_categoria.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['COD_CATE'] ?: null;   
    $dados = [
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO CATEGORIA($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE CATEGORIA SET " . implode(', ', $set_sql) . " WHERE COD_CATE = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_categoria.php?status=success");
        exit;
    } catch (PDOException $e) {
        die("Erro ao salvar tipo de categoria: " . $e->getMessage());
    }
}

// --- LÓGICA DE BUSCA E CONSULTA ---
$sql_base = "SELECT * FROM CATEGORIA";
$sql_count_base = "SELECT COUNT(*) FROM CATEGORIA";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "COD_CATE LIKE ?";
...
$sql_final = $sql_base ." ORDER BY COD_CATE LIMIT $limit OFFSET $offset";
...
$link_primeiro = "cadastro_categoria.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_categoria.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_categoria.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_categoria.php?{$query_string}" . $total_pages;
?>
...
<head>
    ...
    <title>Cadastro de Categorias - Lindomar Despachante</title>
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php endif; ?>
             ...
             <?php 
                if (($action === 'edit') && !empty($categoria_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong> <?= htmlspecialchars($categoria_para_editar['COD_CATE'] ?? '')?><br><br>
                        DESCRIÇÃO: <?= htmlspecialchars($categoria_para_editar['DESCRICAO']?? '') ?>
                    </div>

                    <div class="client-actions">
                        <a href="cadastro_categoria.php" class="btn btn-danger">Voltar para Consulta</a>
                    </div>
                </div>
            ...
            <?php else :?>
                <div class="form-toolbar">
                    <?php if (temPermissao('gerenciar_config')): ?>
                        <a href="cadastro_categoria.php?action=new" class="btn">Novo</a>
                    <?php endif; ?>        
                </div>
            <?php endif; ?>
             ...
             <div class="tabs">
                ...
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?= $aba_ativa == 'consulta_categoria' ? 'active' : '' ?>" data-tab="consulta_categoria" <?php if ($aba_ativa == 'cadastro_categoria'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_categoria' ? 'active' : '' ?>" data-tab="cadastro_categoria" <?= $aba_ativa != 'cadastro_categoria' ? 'disabled' : '' ?>>Cadastro</button>
                </div>

                <div class="tab-content"> 
                <!--consulta-->               
                    <div id="consulta_categoria" class="tab-pane <?= $aba_ativa == 'consulta_categoria' ? 'active' : '' ?> " style="padding: 20px;">
                        <form method="GET" action="cadastro_categoria.php">
                            ...
                            <table class="data-table">
                                ...
                                <tbody>
                                    <?php if (!empty($categoria)): ?>
                                        ...
                                        <tr class="clickable-row" data-id="<?= $categorias['COD_CATE'] ?>">
                                            <td><?= htmlspecialchars($categorias['COD_CATE']??'') ?></td>
                                            <td><?= htmlspecialchars($categorias['DESCRICAO']??'') ?></td>
                                            <td><?= htmlspecialchars($categorias['ABREVIACAO']??'') ?></td>
                                            <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_categoria.php?action=edit&id=<?=$categorias['COD_CATE'] ?>" class="btn btn-primary">Editar</a>
                                                <?php endif; ?>
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_categoria.php?action=delete&id=<?=$categorias['COD_CATE'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                                <?php endif; ?>
                                            </td>      
                                        </tr>
                                        <?php endforeach; ?>
                    ...
                    <!--cadastro-->  
                    <div id="cadastro_categoria" class="tab-pane <?= $aba_ativa == 'cadastro_categoria' ? 'active' : '' ?>">
                        <form id="formCategoria" method="POST" action="cadastro_categoria.php">
                            <input type="hidden" name="COD_CATE" value="<?= htmlspecialchars($categoria_para_editar['COD_CATE'] ?? '') ?>">
                                ...
                                <fieldset class="form-section">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group" style="max-width: 100px;">
                                            <label for="COD_CATE">Código</label>
                                            <input type="text" id="COD_CATE" value="<?= htmlspecialchars($categoria_para_editar['COD_CATE'] ?? 'NOVO') ?>" readonly>
                                        </div>

                                        <div class="form-group">
                                            <label for="descricao_categoria">Descrição</label>
                                            <input type="text" id="descricao_categoria" name="DESCRICAO" value="<?= htmlspecialchars($categoria_para_editar['DESCRICAO'] ?? '') ?>" required>
                                        </div>
                                    </div>
                                </fieldset>
                            </fieldset>
                            ...
                            <div class="form-footer">
                                ...
                                <?php endif; ?>
                                <a href="cadastro_categoria.php" class="btn btn-danger">Cancelar</a>
                            </div>
                        </form>
                    </div>
                </div>
