...
$aba_ativa = 'consulta_combustivel';
$combustivel_para_editar = [];
$combustivel = [];
...
if (!empty($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $message = '<strong>Sucesso!</strong> Combustível salvo com sucesso.';
            $message_type = 'success';
            break;
        case 'deleted':
            $message = 'Processo Combustível com sucesso.';
            $message_type = 'success';
            break;
        case 'error':
...
if ($action === 'new') {
    $aba_ativa = 'cadastro_combustivel';
}

if (($action === 'edit' || $action === 'view') && $id) {
    $aba_ativa = 'cadastro_combustivel';
    $stmt = $conn->prepare("SELECT * FROM COMBUSTIVEL WHERE COD_COMBU = ?");
    $stmt->execute([$id]);
    $combustivel_para_editar = $stmt->fetch(PDO::FETCH_ASSOC);
}

if ($action === 'delete' && $id) {
    try {
        $stmt = $conn->prepare("DELETE FROM COMBUSTIVEL WHERE COD_COMBU = ?");
        $stmt->execute([$id]);
        header("Location: cadastro_combustivel.php?status=deleted");
        exit;
    } catch (PDOException $e) {
        header("Location: cadastro_combustivel.php?status=error&msg=" . urlencode($e->getMessage()));
        exit;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $CODIGO = $_POST['COD_COMBU'] ?: null;   
    $dados = [
    ...
    try {
        if (empty($CODIGO)) {
            ...
            $sql = "INSERT INTO COMBUSTIVEL ($colunas) VALUES ($placeholders)";
            ...
            $sql = "UPDATE COMBUSTIVEL SET " . implode(', ', $set_sql) . " WHERE COD_COMBU = ?";
            $valores = array_values($dados);
            $valores[] = $CODIGO;
            $stmt = $conn->prepare($sql);
            $stmt->execute($valores);
        }
        header("Location: cadastro_combustivel.php?status=success");
        exit;
    } catch (PDOException $e) {
        die("Erro ao salvar tipo de combustível: " . $e->getMessage());
    }
}

// --- LÓGICA DE BUSCA E CONSULTA ---
$sql_base = "SELECT * FROM COMBUSTIVEL";
$sql_count_base = "SELECT COUNT(*) FROM COMBUSTIVEL";
$params = [];
$where_clauses = [];
...
if (!empty($_GET['valor_pesquisa'])) {
    ...
    if ($campo == 'codigo') $where_clauses[] = "COD_COMBU LIKE ?";
...
$sql_final = $sql_base ." ORDER BY COD_COMBU LIMIT $limit OFFSET $offset";
...
$stmt->execute($params);
$combustivel = $stmt->fetchAll(PDO::FETCH_ASSOC);


// Lógica  de navegação
$query_string = http_build_query(array_merge($_GET, ['page' => '']));
$link_primeiro = "cadastro_combustivel.php?{$query_string}1";
$link_anterior = ($page > 1) ? "cadastro_combustivel.php?{$query_string}" . ($page - 1) : "#";
$link_proximo = ($page < $total_pages) ? "cadastro_combustivel.php?{$query_string}" . ($page + 1) : "#";
$link_ultimo = "cadastro_combustivel.php?{$query_string}" . $total_pages;
...
<head>
    ...
    <title>Cadastro de Combustível - Lindomar Despachante</title>
    ...
    <div class="main-container">
        ...
        <main class="main-content">
            ...
            <?php endif; ?>
               ...
               <?php 
                if (($action === 'edit') && !empty($combustivel_para_editar)) :
            ?>
                <div class= "form-tollbar client-header" style="display: flex; justify-content: space-between;">
                    <div class="client-info">
                       <strong>CÓDIGO:</strong> <?= htmlspecialchars($combustivel_para_editar['COD_COMBU'] ?? '')?><br><br>
                        DESCRIÇÃO: <?= htmlspecialchars($combustivel_para_editar['DESCRICAO']?? '') ?>
                    </div>

                    <div class="client-actions">
                        <a href="cadastro_combustivel.php" class="btn btn-danger">Voltar para Consulta</a>
                    </div>
                </div>
            ...
            <?php else :?>
                <div class="form-toolbar">
                    <?php if (temPermissao('gerenciar_config')): ?>
                        <a href="cadastro_combustivel.php?action=new" class="btn">Novo</a>
                    <?php endif; ?>        
                </div>
            <?php endif; ?>
             <div class="tabs">
                <div class="tab-buttons">
                    <button type="button" class="tab-button <?= $aba_ativa == 'consulta_combustivel' ? 'active' : '' ?>" data-tab="consulta_combustivel" <?php if ($aba_ativa == 'cadastro_combustivel'): ?> style="display: none;" <?php endif;?>>Consulta</button>
                    <button type="button" class="tab-button <?= $aba_ativa == 'cadastro_combustivel' ? 'active' : '' ?>" data-tab="cadastro_combustivel"<?= $aba_ativa != 'cadastro_combustivel' ? 'disabled' : '' ?>>Cadastro</button>
                </div>
             

                <div class="tab-content">                
                <div id="consulta_combustivel" class="tab-pane <?= $aba_ativa == 'consulta_combustivel' ? 'active' : '' ?>" style="padding: 20px;">
                    <form method="GET" action="cadastro_combustivel.php">
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (!empty($combustivel)): ?>
                                    <?php foreach ($combustivel as $combustivels): ?>
                                    <tr class="clickable-row" data-id="<?= $combustivels['COD_COMBU'] ?>">
                                        <td><?= htmlspecialchars($combustivels['COD_COMBU']??'') ?></td>
                                        <td><?= htmlspecialchars($combustivels['DESCRICAO']??'') ?></td>
                                        <td><?= htmlspecialchars($combustivels['ABREVIATURA']??'') ?></td>
                                        <td style="text-align: center; white-space: nowrap; gap: 15px; display: flex; justify-content: center;">
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_combustivel.php?action=edit&id=<?=$combustivels['COD_COMBU'] ?>" class="btn btn-primary">Editar</a>
                                                <?php endif; ?>
                                                <?php if (temPermissao('gerenciar_config')) : ?>
                                                    <a href="cadastro_combustivel.php?action=delete&id=<?=$combustivels['COD_COMBU'] ?>" class= "btn btn-danger btn-excluir-linha">Excluir</a>
                                                <?php endif; ?>
                                            </td>  
                                    </tr>
                                    <?php endforeach; ?>
               ...
               <div id="cadastro_combustivel" class="tab-pane <?= $aba_ativa == 'cadastro_combustivel' ? 'active' : '' ?>">
                    <form id="formcombustivel" method="POST" action="cadastro_combustivel.php">
                        <input type="hidden" name="COD_COMBU" value="<?= htmlspecialchars($combustivel_para_editar['COD_COMBU'] ?? '') ?>">
                        <fieldset <?= ($action === 'view') ? 'disabled' : '' ?> style="border: none;">
                            <fieldset class="form-section">
                                <legend>Dados Gerais</legend>
                                <div class="form-row">
                                    <div class="form-group" style="max-width: 100px;">
                                        <label for="COD_COMBU">Código</label>
                                        <input type="text" id="COD_COMBU" value="<?=htmlspecialchars($combustivel_para_editar['COD_COMBU'] ?? 'NOVO') ?>" readonly >
                                    </div>
                                    <div class="form-group">
                                        <label for="DESCRICAO">Descrição</label>
                                        <input type="text" id="DESCRICAO" name="DESCRICAO" value="<?= htmlspecialchars($combustivel_para_editar['DESCRICAO'] ?? '') ?>">
                                    </div>
                                    <div class="form-group">
                                        <label for="ABREVIATURA">Abreviatura</label>
                                        <input type="text" id="ABREVIATURA" name="ABREVIATURA" value="<?= htmlspecialchars($combustivel_para_editar['ABREVIATURA'] ?? '') ?>">
                                    </div>
                                </div>
                            </fieldset>
                        </fieldset>
                        <div class="form-footer">
                            ...
                            <?php endif; ?>
                            <a href="cadastro_combustivel.php" class="btn btn-danger">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
