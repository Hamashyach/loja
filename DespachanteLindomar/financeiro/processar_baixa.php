...
$valor_recebido = isset($_POST['valor_recebido']) ? (float)str_replace(',', '.', $_POST['valor_recebido']) : 0.0;
...
if (empty($fin_raw) || $valor_recebido <= 0) {
    header("Location: ficha_financeira.php?cpf_cnpj=" . urlencode($cpf_cnpj) . "&status=error&msg=" . urlencode("Parâmetros inválidos."));
    exit;
}
...
try {
    ...
    foreach ($fin_ids as $fin_id) {
        ...
        $stmtPago = $conn->prepare("SELECT COALESCE(SUM(CEC_VALOR),0) FROM CAIXA_ITEM WHERE CEC_FIN_CODIGO = ? AND CEC_ESTORNADO = 0 AND CEC_CREDEB = 'C'");
        ...
        $stmtInsItem = $conn->prepare("
            ...
            VALUES (?, NULL, 'C', ?, ?, NULL, ?, ?, 0, ?, ?)
        ");
        $stmtInsItem->execute([$cai_codigo, $valor_para_gravar, $fin_id, $complemento, $fin['FIN_ORS_CODIGO'], $historico, $usuario_id]);

        $stmtUpdCx = $conn->prepare("UPDATE CAIXA SET CAI_VLRCREDITO = COALESCE(CAI_VLRCREDITO,0) + ? WHERE CAI_CODIGO = ?");
        $stmtUpdCx->execute([$valor_para_gravar, $cai_codigo]);

        $novo_pago = $ja_pago + $valor_para_gravar;
        if ($novo_pago >= $valor_alvo) {
    ...
    header("Location: ficha_financeira.php?cpf_cnpj=" . urlencode($cpf_cnpj) . "&status=success&msg=" . urlencode("Baixa registrada com sucesso."));
    exit;

} catch (Exception $e) {
    if ($conn->inTransaction()) $conn->rollBack();
    
    header("Location: ficha_financeira.php?cpf_cnpj=" . urlencode($cpf_cnpj) . "&status=error&msg=" . urlencode($e->getMessage()));
    exit;
}