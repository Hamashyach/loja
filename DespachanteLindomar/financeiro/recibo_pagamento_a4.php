...
$stmt2 = $conn->prepare("
    SELECT 
        COALESCE(SUM(CEC_VALOR), 0) as pago,
...
<head>
<meta charset="utf-8">
<title>Comprovante de Pagamento #<?= $fin ?></title>
...
<div class="paper">
    ...
    <div class="header">
        ...
        <div class="doc-title">
            <h1>COMPROVANTE</h1>
            <p><strong>Nº do Lançamento:</strong> <?= $fin ?></p>
    ...
    <div class="parties">
        ...
        <div class="party-box">
            <h3>RECEBEDOR (FORNECEDOR)</h3>
            <p><strong>Nome:</strong> <?= htmlspecialchars($r['FIN_NOME']) ?></p>
            <p><strong>CPF/CNPJ:</strong> <?= htmlspecialchars($r['CPF_CNPJ']) ?></p>
        </div>
    </div>

    <h2 style="font-size: 1.2em; color: #333; margin-top: 30px; margin-bottom: 10px;">Detalhes do Pagamento</h2>
    ...
    <table class="table-details">
        ...
        <tfoot>
            <tr class="total-row">
                <td colspan="4" class="right">TOTAL PAGO</td>
                <td class="right">R$ <?= number_format($valor_pago, 2, ',', '.') ?></td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="signature-line">
        <div class="line"></div>
        <p><strong>Assinatura do Recebedor (Fornecedor)</strong></p>
        <p><?= htmlspecialchars($r['FIN_NOME']) ?></p>
    </div>
